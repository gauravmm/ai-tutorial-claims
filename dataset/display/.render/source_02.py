...
def render(text: str, _seed: str) -> str:
    ...
    for line in body:
        if line == "-" * 36:
            body_nodes.append(f'<line x1="145" y1="{y - 7}" x2="755" y2="{y - 7}" stroke="#b8b8b8"/>')
            ...
            css = "fine" if line.startswith("Unsubscribe:") else "total" if line.startswith("Total ") else "body"
            body_nodes.append(f'<text x="145" y="{y}" class="{css}">{html.escape(line)}</text>')
        y += 24
