...
def render(text: str, seed: str) -> str:
    ...
    for index, line in enumerate(lines):
        ...
        text_nodes.append(
            f'<text x="{paper_x + 39}" y="{y}" fill="{ink}" fill-opacity="{rng.uniform(0.78, 0.98):.2f}" font-weight="{weight}">{html.escape(line)}</text>'
        )
    ...
    for point in range(18):
        ...
        stain_points.append(
            f"{stain_x + math.cos(theta) * radius:.1f},{stain_y + math.sin(theta) * radius * 0.62:.1f}"
        )
    splotch = "M " + " L ".join(stain_points) + " Z"
    tape_x = paper_x + rng.randint(70, paper_width - 220)
    wood_grain = "".join(
        f'<path d="M 0,{yy} Q 230,{yy + rng.randint(-22, 22)} 450,{yy + rng.randint(-12, 12)} T 900,{yy + rng.randint(-18, 18)}"/>'
        for yy in range(45, 1200, 72)
    )
    speckles = "".join(
        f'<circle cx="{rng.randint(paper_x + 16, paper_x + paper_width - 16)}" cy="{rng.randint(paper_y + 14, paper_y + paper_height - 14)}" r="{rng.uniform(0.4, 2.1):.1f}" opacity="{rng.uniform(0.08, 0.32):.2f}"/>'
        for _ in range(115)
    )
    burn_lines = "".join(
        f'<rect x="{paper_x + rng.randint(18, paper_width - 24)}" y="{paper_y}" width="{rng.randint(1, 5)}" height="{paper_height}" fill="#332d25" opacity="{rng.uniform(0.025, 0.07):.3f}"/>'
        for _ in range(7)
    )
