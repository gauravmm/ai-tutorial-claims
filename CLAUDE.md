...
- ASCII only in Python files, README.md and CLAUDE.md (pre-commit enforces this): no en/em dashes, curly quotes, or other Unicode gremlins. `.claude/` is exempt because it is vendored. `.github/skills/` is exempt because the goose joke skill needs emoji. `PROPOSAL.md` is exempt because it is pre-existing implementer notes.
...
- `dataset/.generate/` is the real source of truth. Hide it from students with `files.exclude` and `search.exclude`. Never put answers in student-facing files.
- `claim.csv` is student output and is gitignored. `dataset/claim.reference.csv` is committed.
- `easter-egg/` holds local goose art and is gitignored except `easter-egg/PROMPT.md`.

## Workflow

- `python3 test_claims.py` is the self-check. It regenerates via `build.py` and diffs against the committed artifacts. Students never run this.
